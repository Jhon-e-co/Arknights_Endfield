...
export default function SquadForm({ mode, initialData, squadId }: SquadFormProps) {
  ...
  useEffect(() => {
    if (initialData && characters.length > 0) {
      setFormData({
      ...
      // Map member IDs to character objects
      const memberCharacters = initialData.members?.map(memberId => {
        return characters.find(c => c.id === memberId) || null;
      }) || [];

      // Fill the rest with null to make it 4 slots
      while (memberCharacters.length < 4) {
        memberCharacters.push(null);
      }
      setSelectedCharacters(memberCharacters.slice(0, 4));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(`Failed to ${mode} squad:`, err);
      setError(`Failed to ${mode} squad. Please try again.`);
      ...
      >
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-black uppercase">
            {mode === 'edit' ? 'EDIT' : 'CREATE'} <span className="bg-[#FCEE21] px-1">SQUAD</span>
          </h1>
        </div>
      </motion.div>
