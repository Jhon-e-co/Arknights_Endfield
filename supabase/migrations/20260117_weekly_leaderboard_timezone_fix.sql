...
-- 周榜时区统一与重置逻辑
-- 创建时间: 2026-01-17
-- 功能: 统一使用美国时区(UTC-5)，每周重置周榜
-- ==========================================

-- 1. 修复周榜触发器 - 使用美国时区(UTC-5)
...
begin
  current_week_start := date_trunc('week', new.created_at at time zone 'UTC-5')::date;
...
declare
  current_week_start date;
begin
  current_week_start := date_trunc('week', now() at time zone 'UTC-5')::date;
