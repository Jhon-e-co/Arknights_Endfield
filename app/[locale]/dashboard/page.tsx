...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const blueprintLikes = allBlueprints?.reduce((sum: number, item: any) => sum + (item.likes || 0), 0) || 0; // eslint-disable-line @typescript-eslint/no-explicit-any
  const squadLikes = allSquadsRaw?.reduce((sum: number, item: any) => sum + (item.likes || 0), 0) || 0; // eslint-disable-line @typescript-eslint/no-explicit-any
  const totalLikes = blueprintLikes + squadLikes;

  // 3. 按需获取数据
  let myBlueprints: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
  let mySquads: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
  let mySavedBlueprints: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
  let mySavedSquads: any[] = []; // eslint-disable-line @typescript-eslint/no-explicit-any
  let dbError: any = null; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  if (activeTab === 'blueprints') {
    ...
    mySavedSquads = rawSavedSquads.map((squad: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
      ...squad,
      members: (squad.members || [])
        .map((id: string) => charMap.get(id))
        .filter((item: any) => item !== undefined) // eslint-disable-line @typescript-eslint/no-explicit-any
    }));
  }
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-zinc-100">
              ...
              <Image
                ...
                alt={t('app.dashboard.page.avatar')}
                fill
          ...
          <div className="space-y-12">
            <section>
              ...
              {mySavedSquads.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {mySavedSquads.map((squad: any) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                    <div key={squad.id}>
            ...
            <section>
              ...
              {mySavedBlueprints.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {mySavedBlueprints.map((bp: any) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                    <div key={bp.id}>
