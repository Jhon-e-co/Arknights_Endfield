...
export async function BlueprintsList({ 
  ...
  if (userId) {
    ...
    userLikes = new Set(likesResult.data?.map((l: any) => l.blueprint_id) || []); // eslint-disable-line @typescript-eslint/no-explicit-any
    userFavorites = new Set(favoritesResult.data?.map((f: any) => f.blueprint_id) || []); // eslint-disable-line @typescript-eslint/no-explicit-any
  }

  const mappedBlueprints: Blueprint[] = (blueprints || []).map((bp: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
    id: bp.id,
    title: bp.title,
